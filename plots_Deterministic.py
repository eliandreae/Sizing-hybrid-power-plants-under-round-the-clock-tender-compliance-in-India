...
import pandas as pd
data = pd.read_csv('Results_HPC/Results_Deterministic_hourly_HPC.csv')
dataY = pd.read_csv('Results_HPC/Results_Deterministic_yearly_HPC.csv')
...
plt.legend(loc='best')
plt.ylim(-1 * pow(10, 8), 3 * pow(10, 8))
...
# Renewable share
fig, ax = plt.subplots()
x = dataY['renewable requirement']
d = dataY['yearly R share']
j = x-d
ax = x.loc[0:24].plot.bar(label='Yearly renewable requirement', rot=0, color='darkslateblue')
ax = d.loc[0:24].plot.bar(label='Yearly renewable energy', rot=0, color='yellowgreen')
ax.set_xlabel('Time [year]')
ax.set_ylabel('Power supply and requirement [MWh]')

...
import pandas as pd
def function(CO2_cost = None, PPA_init = 70, future_elec_price_change = None):
    Nyears = 30
    from clean_data_NEW import get_clean_data
    all_data, Ts, Y, Yp, Years, Hours_in_year = get_clean_data(Nyears,
                                                               future_elec_price_change=future_elec_price_change)
    ...
    opex_refurbish_bat, opex_refurbish_E, init = Parameters(Nyears, PPA_init=PPA_init)

    if CO2_cost is None:
        CO2_cost = {y: 0 for y in Years}

    ####################################################################################################################

    def Investment(CO2_cost = None):
        m = pyo.ConcreteModel()
        ...
        m.NPV = pyo.Objective(expr=sum((1/pow((1 + r), Y[year])) * (m.Revenue[year] - m.Prod[year]
        ...
        for year in Years:
            m.PROD.add(pyo.quicksum(solarC * m.solarP[t]
                                    + windC * m.windP[t]
                                    + ((coalFuelC[t-1] + CO2_cost[year] * coalCO2Emis)/eta_fuel) * m.coalP[t]
        ...
        for year in Years:
            m.yearly_notMetTender.add(
                ...
                >= tau * len(Hours_in_year[year]) - m.e_nonDelivered[year])

        return m

    m = Investment(CO2_cost=CO2_cost)

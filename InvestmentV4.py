...
def Investment():
    ...
    m.PENALTY = pyo.Constraint(
        ...
        penalty_utilization_yearly_dyn[year] * m.e_nonDel_yearly[year] <= m.Penalty[year])
...
file1 = results_hourly.to_csv('Results_HPC/Results_Investment_hourly_HPC.csv', index=False, header=True)
file2 = results_yearly.to_csv('Results_HPC/Results_Investment_yearly_HPC.csv', index=False, header=True)
file3 = optimal_capacities.to_csv('Results_HPC/Results_Investment_capacities_HPC.csv', index=False, header=True)

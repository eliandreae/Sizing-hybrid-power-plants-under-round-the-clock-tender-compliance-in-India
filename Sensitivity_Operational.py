...
def function1(tau_s = 250, solar_max = 300, wind_max = 400, coal_max = 250, battery_max = 250,
              energy_max = 300):
    Nyears = 1
    ...
    def Economic_dispatch(tau_s = 250, solar_max = 300, wind_max = 400, coal_max = 250,
                          battery_max = 250, energy_max = 300):
        m = pyo.ConcreteModel()
        ...
        m.PV = pyo.Constraint(Ts, rule=lambda m, t: m.solarP[t] <= float(CFsolar[t-1]) * solar_max)
        m.Turbine = pyo.Constraint(Ts, rule=lambda m, t: m.windP[t] <= float(CFwind[t-1]) * wind_max)
    ...
    m = Economic_dispatch(tau_s=tau_s, solar_max=solar_max, wind_max=wind_max, coal_max=coal_max,

...
import pandas as pd
def function(tau_s = 250, bat_scale = 1, renew_yearly = 0.51, util_eff_yearly = 0.85, d_rate = 0.11):
    Nyears = 30
...
########################################################################################################################
    ...
    def Deterministic(tau_s = 250, bat_scale = 1, renew_yearly = 0.51, util_eff_yearly = 0.85, d_rate = 0.11):
        ...
        m.coalPmax = pyo.Var(domain=pyo.NonNegativeReals, bounds=(0, tau_s))
        m.batteryPmax = pyo.Var(domain=pyo.NonNegativeReals, bounds=(0, tau_s))
        ...
        m.NPV = pyo.Objective(expr=sum((1/pow((1 + d_rate), Y[year])) * (m.Revenue[year] - m.Prod[year]
                                                                         - m.OpexVar[year] - m.OpexFix[year]
                                                                         - m.Penalty[year]) for year in Years) - m.Capex
                              , sense=pyo.maximize)

        m.PresentValue = pyo.ConstraintList()
        for year in Years:
            m.PresentValue.add((1 / pow((1 + r), Y[year])) * (m.Revenue[year] - m.Prod[year] - m.OpexVar[year]
                                                              - m.OpexFix[year] - m.Penalty[year]) == m.present_value[
                                   year])
        ...
        m.CAPEX = pyo.Constraint(
            rule=lambda m:
            solarCapex * m.solarPmax + windCapex * m.windPmax
            + coalCapex * m.coalPmax
            + bat_scale * batteryCapex * m.batteryPmax
            + ECapex * m.Emax
            + bat_scale * BOS * m.batteryPmax
            == m.Capex)
        ...
        for year in Years:
            m.OPEXfix.add(solarOpexFix[year] * m.solarPmax
                          ...
                          + bat_scale * batteryOpexFix[year] * m.batteryPmax
                          + EOpexFix[year] * m.Emax
                          + bat_scale * opex_refurbish_bat[year] * m.batteryPmax
        ...
        for year in Years:
            m.OPEXvar.add(pyo.quicksum(solarOpexVar * m.solarP[t]
                                       ...
                                       + bat_scale * batteryOpexVar * m.batteryP[t] for t in Hours_in_year[year])
                          <= m.OpexVar[year])
        ...
        for year in Years:
            m.yearly_renewableShare.add(
                ...
                >= pyo.quicksum(m.totProd[t] for t in Hours_in_year[year]) * renew_yearly - m.e_renew_share[year])

        m.yearly_notMetTender = pyo.ConstraintList()
        for year in Years:
            m.yearly_notMetTender.add(
                pyo.quicksum(m.tenderProd[t] for t in Hours_in_year[year])
                >= util_eff_yearly * tau * len(Hours_in_year[year]) - m.e_nonDel_yearly[year])

        return m

    m = Deterministic(tau_s=tau_s, bat_scale=bat_scale, renew_yearly=renew_yearly,
                      util_eff_yearly=util_eff_yearly, d_rate=d_rate)
    solver_parameters = "ResultFile=model.ilp"
    results = pyo.SolverFactory('gurobi').solve(m, options_string=solver_parameters).write()

    ####################################################################################################################
    # Calculation of LCOE
    LCOE = (m.Capex.value + sum(
        ((m.OpexFix[year].value + m.OpexVar[year].value + m.Prod[year].value) / pow((1 + d_rate), Y[year]))
    ...
    return results_hourly, results_yearly, optimal_capacities

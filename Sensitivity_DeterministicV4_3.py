...
import pandas as pd
def function(penalty_share_util = 4, penalty_share_ren = 4):
    Nyears = 30
    ...
    opex_refurbish_bat, opex_refurbish_E, init = Parameters(Nyears, penalty_share_util=penalty_share_util,
                                                            penalty_share_ren=penalty_share_ren)
    ...
    def Deterministic():
        ...
        for year in Years:
            m.PresentValue.add((1/pow((1 + r), Y[year])) * (m.Revenue[year] - m.Prod[year] - m.OpexVar[year]
                                                            - m.OpexFix[year] - m.Penalty[year])
                               == m.present_value[year])
    ...
    ####################################################################################################################
    # Calculation of LCOE
    LCOE = (m.Capex.value + sum(((m.OpexFix[year].value + m.OpexVar[year].value + m.Prod[year].value)/pow((1 + r),
                                                                                                          Y[year]))

...
import pandas as pd
def function(tau_s = 250, bat_scale = 1, d_rate = 0.11):
    Nyears = 30
    ...
    def Investment(tau_s = 250, bat_scale = 1, d_rate = 0.11):
        m = pyo.ConcreteModel()
        ...
        m.e_nonDelivered = pyo.Var(Years, domain=pyo.NonNegativeReals)
        ...
        m.PENALTY = pyo.Constraint(
            ...
            penalty_utilization_yearly_dyn[year] * m.e_nonDelivered[year] <= m.Penalty[year])
        ...
        for year in Years:
            m.yearly_notMetTender.add(
                ...
                >= tau_s * len(Hours_in_year[year]) - m.e_nonDelivered[year])

        return m

    m = Investment(tau_s=tau_s, bat_scale=bat_scale, d_rate=d_rate)
    ...
    LCOE = (m.Capex.value + sum(
        ((m.OpexFix[year].value + m.OpexVar[year].value + m.Prod[year].value) / pow((1 + r), Y[year]))
    ...
    results_yearly = pd.DataFrame.from_dict({y: {'nonDelivered': m.e_nonDelivered[y].value,

import pandas as pd
import Sensitivity_DeterministicV4_2 as Det
...
for case_label, CO2_cost in cases.items():
    ...
    results_hourly, results_yearly, optimal_capacities = Det.function(CO2_cost=CO2_cost)

    file1 = results_hourly.to_csv('Sensitivity/CO2_cost/Deterministic_CO2_cost_hourly_' + str(case_label) + '.csv',
                                  index=False, header=True)
    file2 = results_yearly.to_csv('Sensitivity/CO2_cost/Deterministic_CO2_cost_yearly_' + str(case_label) + '.csv',
                                  index=False, header=True)
...
file3 = CO2_cost_sensitivity_df.to_csv('Sensitivity/CO2_cost/Deterministic_CO2_cost_overall.csv',
                                       index=False, header=True)
...
for future_elec_price_change in [1.02, 1.04, 1.0609, 1.08, 1.1, 1.12]:
    ...
    results_hourly, results_yearly, optimal_capacities = Det.function(future_elec_price_change=future_elec_price_change)

    file4 = results_hourly.to_csv('Sensitivity/Future_electricity_price/Deterministic_future_elec_price_hourly_'
                                  + str(future_elec_price_change) + '.csv', index=False, header=True)
    file5 = results_yearly.to_csv('Sensitivity/Future_electricity_price/Deterministic_future_elec_price_yearly_'
                                  + str(future_elec_price_change) + '.csv', index=False, header=True)
...
file6 = future_elec_price_change_sensitivity_df.to_csv('Sensitivity/Future_electricity_price/'
                                                       'Deterministic_future_elec_price_overall.csv',
                                                       index=False, header=True)
...
for PPA_init in [30, 50, 70, 90, 110, 130]:
    ...
    results_hourly, results_yearly, optimal_capacities = Det.function(PPA_init=PPA_init)

    file7 = results_hourly.to_csv('Sensitivity/PPA/Deterministic_PPA_init_hourly_' + str(PPA_init) + '.csv',
                                  index=False, header=True)
    file8 = results_yearly.to_csv('Sensitivity/PPA/Deterministic_PPA_init_yearly_' + str(PPA_init) + '.csv',
                                  index=False, header=True)
...
file9 = PPA_init_sensitivity_df.to_csv('Sensitivity/PPA/Deterministic_PPA_init_overall.csv', index=False, header=True)
